...
const HomePage = () => {
  ...
  const { API, gender, user, logout, language, themeClass, isAuthenticated, isBarber } = useApp();
  ...
  const fetchCities = async () => {
    try {
      ...
      // Get cities from all countries for now
      const allCities = [];
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {barber.services?.slice(0, 3).map((service, i) => (
            <span 
              key={i} 
              className={`service-tag text-xs ${isMen ? 'bg-[#1F1F1F] border-[#262626] text-[#94A3B8]' : 'bg-[#FAFAFA] border-[#E7E5E4] text-[#57534E]'}`}
            >
