...
agent_communication:
    ...
    - agent: "testing"
    - message: "✅ BARBER HUB v3.6.1 MEDIUM-PRIORITY VERIFICATION — ALL CRITICAL BEHAVIORS PASS. Password policy correctly rejects weak (<8 char) and no-digit passwords on both /auth/register and /auth/register-barbershop (422 with actionable message), accepts compliant ones. /auth/change-password covers the full matrix: 401 unauthenticated, 400 wrong old, 422 weak new, 400 old==new, 200 happy path, 429 after 11 attempts (rate limit active). must_change_password gate blocks protected endpoints (GET /favorites/my → 403) and auto-clears on successful password rotation. Seed rotation verified: all 10 shops receive unique 16+ char passwords with digits; legacy 'salon123' login now rejected for freshly-seeded shops; login with returned rotated password succeeds. Admin rotation: legacy admin123 no longer accepted; 'NewStrong2026!xYz' works with must_change_password=false. Stripe removal clean — no import errors in logs, /api/health still 200. Zero regressions across /api/health, /api/config/public, /api/products/featured, /api/barbers, /api/ranking/tiers, IDOR guard on /api/bookings/{id}, seed-token lock. System security posture significantly improved and ready for frontend UX work / production deployment."
